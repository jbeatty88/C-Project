...
char InputStream::Next(size_t n, size_t line, size_t idx) {
    ...
    nextVal = idx + n;
// ########################################################
    // // If we are at the very last line
    // if(line + 1 == inVec.size()) {
    //     // If our next index takes us to the end of the line
    //     if(nextVal == length) {
    //         // Return everything up until the newline
    //         return inVec[line][nextVal - 1];
    //     }
    ...
    // else {
    //     // If there is still space on the line
    //     if(nextVal < length) {
    //         // Return what is at that idx
    //         return inVec[line][nextVal];
    //     }
    //     // If there is NOT enough space, go to next line and update our deisred idx
    //     else {
...
// #################################################################3
    ...
    // If there is NOT more space on the line
    else {
        // If we are NOT at the last line of the vector
        if(line + 1 < inVec.size()) {
            return inVec[line + 1][n - ((inVec[line].length() + 1) - currentIdx)];
        }
        else if(line+ 1 == inVec.size()) {
            return inVec[line][idx];
        }
        else {
            isEnd = true;
            return '\0';
...
size_t InputStream::GetCurrentLineIdx() { return currentLine; }
