...
void Lexer::KeywordMachine(char ch) {
    ...
    if(str != "") {
        ...
        if(!isalnum(input.Next(currentStr.length())) && !isalnum(input.Before(1)) && currentStr.size() >= 5) {
            TokenHandler(type);
            return;
        }
        return;
...
std::string Lexer::GetString() {
     ...
     //size_t tmpIdx = input.GetCurrentIdx();
     //size_t newLine(0);
     size_t i(1);
     ss << '\'';
     ...
     while(!isEnd) {
         ...
         // as long as we dont have a single quote or 2 single quotes
             ...
             if(input.Next(i) != ENDFILE_CHAR) {
                ...
                //         ss << input.Next(i);
                //         newLine++;
                //         input.SetCurrentPos(input.GetCurrentLine(), 0);
                //         i = 0;
                //     }
                //     else {
                //         isEnd = true;
